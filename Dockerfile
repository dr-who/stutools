FROM almalinux
MAINTAINER Stu
RUN yum -y install cmake libaio-devel pciutils-devel numactl-devel readline-devel git gcc gcc-c++ make iproute emacs-nox libuuid-devel
RUN yum -y install --enablerepo highavailability pcs pacemaker corosync
RUN yum -y install man man-pages nc jq gdb tmux
