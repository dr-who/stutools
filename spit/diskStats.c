...
void diskStatSummary(diskStatType *d, size_t *totalReadBytes, size_t *totalWriteBytes, size_t *totalReadIO,
                     size_t *totalWriteIO, double *util, size_t shouldReadBytes, size_t shouldWriteBytes, int verbose,
                     double elapsed) {
    if (!d) return;
    ...
    *util = 100.0 * ((d->finishSecTimeio - d->startSecTimeio) / 1000.0 / d->allocDevices) / elapsed;
    ...
    if (verbose && (shouldReadBytes || shouldWriteBytes)) {
        if (*totalReadBytes)
            fprintf(stderr,
                    "*info* read  amplification: should be %zd (%.3lf GiB), actual read  %zd (%.3lf GiB), %.2lf%%, %zd device(s)\n",
                    shouldReadBytes, TOGiB(shouldReadBytes), *totalReadBytes, TOGiB(*totalReadBytes),
                    *totalReadBytes * 100.0 / shouldReadBytes, d->allocDevices);
        if (*totalWriteBytes)
            fprintf(stderr,
                    "*info* write amplification: should be %zd (%.3lf GiB), actual write %zd (%.3lf GiB), %.2lf%%, %zd device(s)\n",
                    shouldWriteBytes, TOGiB(shouldWriteBytes), *totalWriteBytes, TOGiB(*totalWriteBytes),
                    *totalWriteBytes * 100.0 / shouldWriteBytes, d->allocDevices);
        if (d->allocDevices)
...
void diskStatLoadProc(diskStatType *d) {
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        sscanf(line, "%ld %ld %s %zu %ld %zu %ld %zu %ld %zu %ld %ld %zu %zu", &mj, &mn, str, &readcompl1, &s, &read1,
               &s, &writecompl1, &s, &write1, &s, &inflight, &timespentIO, &discardcount1);
        //    fprintf(stderr,"==%zu\n", discardcount1);
