...
static void *runThreadTimer(void *arg) {
  ...
  if (fpmysql) {
    fprintf(fpmysql, "create table if not exists benchmarks (id int not null auto_increment, date datetime not null, blockdevice char(30) not null, read_mbs float not null, read_iops float not null, write_mbs float not null, write_iops float not null, command char(100) not null, version char(10) not null, machine char(20) not null, iotype char(10) not null, opsize char(10) not null, iopattern char(10) not null, qd int not null, devicestate char(15) not null, threads int not null, read_total_gb float not null, write_total_gb float not null, tool char(10) not null, runtime float not null, mysqloptions char(200) not null, os char(20) not null, degraded int not null, k int not null, m int not null, checksum char(15), encryption char(10), cache int not null, machineUptime int, totalRAM bigint, precondition char(50), primary key(id));\n");
...
void jobRunThreads(jobType *job, const int num, char *filePrefix,
  ...
  for (size_t i = 0; i < num; i++) {
    ...
    // use 1/4 of free RAM
    size_t useRAM = MIN((size_t)15L*1024*1024*1024, freeRAM() / 4); // 1L*1024*1024*1024;
