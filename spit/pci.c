...
void listPCIdevices(size_t classfilter) {
	...
	sprintf(strw, "/sys/bus/pci/devices/%04x:%02x:%02x.%d/max_link_width", dev->domain, dev->bus, dev->dev, dev->func);
      ...
      float maxlinkspeed = 1, maxlinkwidth = 1;
      
            FILE *fp = fopen (str, "rt");
      if (fp) {
	char result[100];
	fscanf(fp, "%s", result);
	maxlinkspeed = atof(result);
	printf(", maxLnkSpe %s", result);
      fclose(fp);
      } else {
	//	perror(str);
      }
	...
	char result[100];
	fscanf(fp, "%s", result);
	maxlinkwidth = atof(result);
	printf(", maxLinkWid = %s", result);
      fclose(fp);
      } else {
	//	perror(str);
      }

      printf(", %.1lf Gb/s", maxlinkwidth * maxlinkspeed * 128.0/130);
