...
void latencyOverTime(positionContainer *origpc) {
    if (origpc) {
        ...
        fprintf(fp, "set grid\n");
        fprintf(fp,
                "plot '<sort -k 1n t_vs_lat.txt | grep -w W' using 1:2 with lines title 'Latency over time (writes)', '<sort -k 1n t_vs_lat.txt | grep -w R' using 1:2 with lines title 'Latency over time (reads)'\n");
        fclose(fp);
        ...
        for (size_t i = 0; i < origpc->sz; i++) {
            positionType *p = &origpc->positions[i];
            if (p->finishTime > 0) {
                fprintf(fp, "%lf %lf %c %d\n", p->submitTime - lowesttime, p->finishTime - p->submitTime, p->action,
                        p->len);
            }
        }
        fclose(fp);
...
void latencyLenVsLatency(positionContainer *origpc, int num) {
    ...
    size_t count = 0;
    for (int n = 0; n < num; n++) {
        for (int i = 0; i < (int) origpc[n].sz; i++)
            if (origpc[n].positions[i].finishTime > 0) {
...
void latencyStats(latencyType *lat) {
    ...
    if (histCount(&lat->histRead)) {
        ...
        fprintf(stderr,
                "*info* read latency: n = %zd, mean = %.3lf ms (sd = %.2lf ms), median = %.2lf ms, 99.9%% <= %.2lf ms, 99.99%% <= %.2lf ms, 99.999%% <= %.2lf ms\n",
    ...
    if (histCount(&lat->histWrite)) {
        ...
        if (n < 1000000) five9 = NAN;
        fprintf(stderr,
                "*info* write latency: n = %zd, mean = %.3lf ms (sd = %.2lf ms), median = %.2lf ms, 99.9%% <= %.2lf ms, 99.99%% <= %.2lf ms, 99.999%% <= %.2lf ms\n",
...
void latencyReadGnuplot(latencyType *lat) {
    ...
    if (fp) {
        ...
        fprintf(fp,
                "plot 'spit-latency-histogram-read.txt' using 1:2 with imp title 'Read Latency', 'spit-latency-histogram-read.txt' using 1:3 with lines title '%% Confidence' axes x1y2,'<echo %lf 100000' with imp title 'ART=%.3lf' axes x1y2, '<echo %lf 100000' with imp title '99.9%%=%.2lf' axes x1y2, '<echo %lf 100000' with imp title '99.99%%=%.2lf' axes x1y2, '<echo %lf 100000' with imp title '99.999%%=%.2lf' axes x1y2\n",
