...
int main(int argc, char *argv[]) {
    ...
    fprintf(stderr, "*info* bd size %zd (%.3lf GiB), block low %zd, block high %zd, step %zd, pos '%s'\n", bdSize,
            TOGiB(bdSize), blockLow, blockHigh, stepSize, posfile);
    for (size_t pos = startByte; pos < finishByte;) {
        ...
        pc.positions[count].submitTime = timeAsDouble();
        ...
        *uuiddest = pc.UUID;
        ...
        pc.positions[count].finishTime = timeAsDouble();
