...
void zapfunc(size_t pos, size_t deviceCount, int *selection, size_t blocksize, size_t writeblocksize, int print,
             char *block) {
    if (blocksize) {}
...
void xorfunc(size_t pos, size_t deviceCount, int *selection, size_t blocksize, size_t writeblocksize, int print,
             char *block) {
    if (blocksize) {}
    if (print) fprintf(stderr, "%9x (%5.3lf GiB):  ", (unsigned int) pos, TOGiB(pos));
...
void decimalfunc(size_t pos, size_t deviceCount, int *selection, size_t blocksize, size_t writeblocksize, int print,
                 char *block) {
    if (blocksize) {}
    if (print) fprintf(stderr, "%9x (%5.3lf GiB):  ", (unsigned int) pos, TOGiB(pos));
...
void rotatefunc(size_t pos, size_t deviceCount, int *selection, int *rotated, size_t blocksize, size_t writeblocksize,
                int print, char *block) {
    if (writeblocksize) {}
...
int main(int argc, char *argv[]) {
    ...
    if (deviceCount) {
        ...
        fprintf(stdout, "   raidcorrupt -k 4 -m 0-2  # pick between 0 and 2 devices every 256KiB\n");
        ...
        fprintf(stdout,
                "             Step through all devices in 512 KiB steps, setting the first 4096 bytes to 'Z'\n");
    ...
    if (numOpenDevices(deviceList, deviceCount) < deviceCount) {
        ...
        for (long long pos = startAt, pr = 0; pos < finishAt; pos += blocksize, pr++) {
            ...
            if (zap) {
                ...
                rotatefunc(pos, deviceCount, selection, rotated, blocksize, writeblocksize, (pr % printevery) == 0,
                           block);
            }
        }
        free(block);
