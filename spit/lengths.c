...
void lengthsAdd(lengthsType *l, const size_t len, size_t freq) {
    const size_t maxlen = (1LL << (sizeof(unsigned int) * 8)) - 1;
    if (len > maxlen) {
        fprintf(stderr, "*error* block length is too large (%zd), max is %zd (k%zd or M%zd)\n", len, maxlen,
                maxlen / 1024, maxlen / 1024 / 1024);
        exit(1);
    }
    if (freq < 1) freq = 1;
    l->size++;
...
void lengthsDump(const lengthsType *l, const char *prefix) {
    size_t allo = NAME_MAX + (NAME_MAX * l->size);
    char *s = malloc(allo * sizeof(char));
    assert(s);
    char *starts = s;

    s += sprintf(s, "*info* '%s' lengths dump (min %zd, max %zd)\n", prefix, l->min, l->max);
    int prnt = 1;
    for (size_t i = 0; i < l->size; i++) {
        if (i > 10) {
            prnt = 0;
        ...
        if (prnt) {
            s += sprintf(s, "*info*   [%2zd]\t %zd (%.03lf MiB)\t %zd\n", i, l->len[i], TOMiB(l->len[i]), l->freq[i]);
        } else {
            if (i == 11) {
                s += sprintf(s, "  ...\n");
            }
