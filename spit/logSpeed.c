...
void logSpeedDump(logSpeedType *l, const char *fn, const int format, const char *description, size_t bdSize,
                  size_t origBdSize, double rwratio, size_t flushing, size_t seqFiles, size_t lowbs, size_t highbs,
                  const char *cli) {
    //  logSpeedMedian(l);
    //  if (format) {
    ...
    if (format == MYSQL) {
        ...
        fprintf(fp,
                "create table if not exists runs (id int auto_increment, primary key(id), time datetime, hostname text, domainname text, RAM int, threads int, bdSize float, origBdSize float,rw float, flush int, seqFiles int, lowbs int, highbs int, runtime float, mib float, iops int, description text, cli text);\n"
        ...
        fprintf(fp,
                "insert into runs(description, time, hostname, domainname, RAM, threads, bdSize, origBdSize, rw, flush, seqFiles, lowbs, highbs, cli, runtime, mib, iops) values (\"%s\", NOW(), \"%s\", \"%s\", %zd, %zd, %.1lf, %.1lf, %.1lf, %zd, %zd, %zd, %zd, \"%s\", %.1lf, %zd, %zd);\n",
                (description == NULL) ? "" : description, hostname, domainname, (size_t) (TOGiB(totalRAM()) + 0.5),
                numThreads(), TOGiB(bdSize), TOGiB(origBdSize), rwratio, flushing, seqFiles, lowbs, highbs, cli,
                runtime, mib, iops);
        ...
        for (size_t i = 0; i < l->num; i++) {
            ...
            counttotal += l->rawcount[i];
            fprintf(fp,
                    "insert into rawvalues(id, time,globaltime,MiB,SumMiB,IOs,SumIOs) VALUES(last_insert_id(), %.6lf,%.6lf,%.2lf,%.2lf,%zd,%zd);\n",
                    l->rawtime[i] - l->starttime, l->rawtime[i], l->rawvalues[i], valuetotal, l->rawcount[i],
                    counttotal);
        }
        ...
        for (size_t i = 0; i < l->num; i++) {
            valuetotal += l->rawvalues[i];
            counttotal += l->rawcount[i];
            fprintf(fp, "%.6lf\t%.6lf\t%.2lf\t%.2lf\t%zd\t%zd\n", l->rawtime[i] - l->starttime, l->rawtime[i],
                    l->rawvalues[i], valuetotal, l->rawcount[i], counttotal);
        }
    } else if (format == JSON) {
        fprintf(fp, "[\n");
        for (size_t i = 0; i < l->num; i++) {
            ...
            counttotal += l->rawcount[i];
            fprintf(fp,
                    "{\"time\":\"%.6lf\", \"globaltime\":\"%.6lf\", \"MiB\":\"%.2lf\", \"SumMiB\":\"%.2lf\", \"IOs\":\"%zd\", \"SumIOs\":\"%zd\"}",
                    l->rawtime[i] - l->starttime, l->rawtime[i], l->rawvalues[i], valuetotal, l->rawcount[i],
                    counttotal);
