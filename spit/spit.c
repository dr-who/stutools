...
void usage() {
  ...
  fprintf(stderr,"  spit ... -c ws0u -v           # Generates r/w pairs with unique seeds, tests with QD=1\n");
  fprintf(stderr,"  spit ... -c ws0uG_ -v -j32    # Generates r/w pairs with unique seeds, as above, unique thread ranges\n");
  exit(0);
}
