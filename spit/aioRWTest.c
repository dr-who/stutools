...
#define VERSION __TIMESTAMP__
  ...
  if (table) {
	...
	for (size_t qdindex=0; qdindex < qdSpecified; qdindex++) {
	  for (size_t bsindex=0; bsindex < sizeof(bsArray) / sizeof(bsArray[0]); bsindex++) {
	    ...
	    if (ssArray[ssindex] == 0) {
	      ...
	      setupPositions(positions, &maxPositions, 0, rrArray[rrindex], bsArray[bsindex], bsArray[bsindex], alignment, startAtZero, bdSizeWeAreUsing, &cigar, seed);
	      ...
	      setupPositions(positions, &maxPositions, ssArray[ssindex], rrArray[rrindex], bsArray[bsindex], bsArray[bsindex], alignment, startAtZero, bdSizeWeAreUsing, &cigar, seed);
    ...
    setupPositions(positions, &maxPositions, seqFiles, readRatio, LOWBLKSIZE, BLKSIZE, alignment, startAtZero, bdSizeWeAreUsing, &cigar, seed);
