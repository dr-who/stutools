...
size_t aioMultiplePositions(positionContainer *p,
    ...
    *posCompleted = 0;
    ...
    while (keepRunning && thiskeeprunning) {
                    ...
                    if (positions[pos].action != 'S') { // if we have some positions, sz > 0
                        ...
                        if (positions[pos].action == 'T') {
                            if (discard_max_bytes >= alignment) {
                                ...
                                positions[pos].submitTime = timeAsDouble();
                                performDiscard(fd, NULL, newpos, newpos + len, maxSize, alignment, NULL, 0, 0);
                                p->writtenIOs++;
                                positions[pos].finishTime = timeAsDouble();
                                ...
                                if (recordSamples && positions[pos].samples < recordSamples) {
                                    ...
                                    positions[pos].latencies[pppp] =
                                            positions[pos].finishTime - positions[pos].submitTime;
                                }
                            }
                        ...
                        if (fd >= 0) {
                            ...
                            if (ret > 0) {
                                ...
                                if (verbose >= 2 || (newpos & (alignment - 1))) {
                                    fprintf(stderr,
                                            "fd %d, pos %zd (%% %zd = %zd ... %s), size %zd, inFlight %zd, QD %zd, submitted %zd, received %zd\n",
                                            fd, newpos, alignment, newpos % alignment,
                                            (newpos % alignment) ? "NO!!" : "aligned", len, inFlight, QD, submitted,
                                            received);
                                }
        ...
        double timeelapsed = timeAsDouble() - last;
            ...
            if (!tableMode) {
                if (verbose != -1) {
                    ...
                    fprintf(stderr, "[%.1lf] %.1lf GB, qd: %zd, op: %zd, [%zd], %.0lf IO/s, %.1lf MB/s\n",
                            thistime - start, TOGB(totalReadBytes + totalWriteBytes), inFlight, received, pos, IOspeed,
                            speed);
                }
                if (verbose >= 2) {
                    if (flush_count)
                        fprintf(stderr, "*info* avg flush time %.4lf (min %.4lf, max %.4lf)\n",
        ...
        if (ret > 0) {
            double lastreceive = timeAsDouble();
		    ...
		    // if successful read or write
                    ...
                    if (pp->verify && (pp->action == 'R')) {
                        ...
                        if (stringcompare) {
                            strcmpres = strncmp(readdata[pp->q] + 16, data[pp->q] + 16, pp->len - 16) ||
                                        (pp->pos != *poscheck);
                        } else {
                        ...
                        if (((firstcmp) || (strcmpres != 0)) && (positions[pp->verify].finishTime)) {
                            ...
                            fprintf(stderr,
                                    "*error* position[%zd] cmp=%d, '%c' R=%d (success %d) ver=%d wrong. UUID %zd/%zd, pos %zd/%zd\n",
		  ...
		  if (exitOnErrors) {
		    ...
		    exit(-1);
                ...
                if (pp->finishTime - pp->submitTime > 30) {
                    ...
                    char s[NAME_MAX];
                    sprintf(s,
                            "slow I/O (%c,pos=%zd,size=%d) %.1lf s, submission loop, %zd slow from %zd submitted (%.1lf%%)\n",
                            pp->action, pp->pos, pp->len, pp->finishTime, slow, submitted,
                            slow * 100.0 / (slow + submitted));
                    syslogString("spit", s);
            ...
            if (ret > 0) {
                for (int j = 0; j < ret; j++) {
                    ...
                    if (pp->finishTime - pp->submitTime > 30) {
                        ...
                        char s[NAME_MAX];
                        sprintf(s,
                                "slow I/O (%c,pos=%zd, size=%d) %.1lf s, no submission/post loop, %zd slow from %zd submitted (%.1lf%%)\n",
                                pp->action, pp->pos, pp->len, pp->finishTime, slow, submitted,
                                slow * 100.0 / (slow + submitted));
                        syslogString("spit", s);
                        fprintf(stderr, "*warning* %s", s);
                ...
                if (count > 5 && (timeAsDouble() - lastprint >= 3)) {
                    fprintf(stderr, "*warning* waiting for %zd IOs in flight, iteration %zd, %zd seconds...\n",
                            inFlight, count, (size_t) (
                                    timeAsDouble() - snaptime));
                    lastprint = timeAsDouble();
                }
                usleep(10000);
            }
        }
    ...
    if (inFlight) {
        fprintf(stderr, "*warning* timed out after %.0lf seconds. Flight requests still = %zd\n",
                timeAsDouble() - snaptime, inFlight);
    }
        ...
        if (positions[i].submitTime > 0) {
            if (positions[i].inFlight) {
                fprintf(stderr, "*warning* position %zd inflight! submit %lf, success %d\n", positions[i].pos,
                        positions[i].submitTime, positions[i].success);
            }
        }
    }
